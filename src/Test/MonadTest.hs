...
bindReaderTest = testCase "(=<<) for (->)" $ ((*) =<< (+ 10)) 7 @?= 119

appTest :: TestTree
appTest = testGroup
  "<**>"
  [ testCase "ExactlyOne" $ ExactlyOne (+ 10) <**> ExactlyOne 8 @?= ExactlyOne
    18
  , testCase "List"
  $    (+ 1)
  :.   (* 2)
  ...
  :.   Nil
  @?=  (2 :. 3 :. 4 :. 2 :. 4 :. 6 :. Nil)
  , testCase "Optional" $ Full (+ 8) <**> Full 7 @?= Full 15
  , testCase "Optional - empty function"
  $    Empty
  <**> Full 7
  @?=  (Empty :: Optional Integer)
  , testCase "Optional - empty value" $ Full (+ 8) <**> Empty @?= Empty
  , testCase "(->) 1" $ ((+) <**> (+ 10)) 3 @?= 16
  , testCase "(->) 2" $ ((+) <**> (+ 5)) 3 @?= 11
  , testCase "(->) 3" $ ((+) <**> (+ 5)) 1 @?= 7
  , testCase "(->) 4" $ ((*) <**> (+ 10)) 3 @?= 39
...
joinTest = testGroup
  ...
  $   join (Full Empty)
  @?= (Empty :: Optional Integer)
  , testCase "Optional all Full" $ join (Full (Full 7)) @?= Full 7
  , testCase "(->)" $ join (+) 7 @?= 14
  ]

bindFlippedTest :: TestTree
bindFlippedTest = testCase "(>>=)" $ ((+ 10) >>= (*)) 7 @?= 119
