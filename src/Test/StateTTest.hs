...
import           Course.StateT                  ( Logger(Logger)
                                                , OptionalT(OptionalT)
...
applicativeTest = testGroup
  ...
  @?= ((4, 0) :. Nil)
  , testCase "Optional"
    $ let st = StateT (\s -> Full ((+ 2), s P.++ [1]))
            <*> StateT (\s -> Full (2, s P.++ [2]))
...
monadTest = testGroup
  ...
  , testCase "modify"
    $ let modify f = StateT (\s -> pure ((), f s))
      in  runStateT (modify (+ 1) >>= \() -> modify (* 2)) 7
            @?= (((), 16) :. Nil)
  ]

state'Test :: TestTree
state'Test =
  testCase "state'" $ runStateT (state' . runState $ put 1) 0 @?= ExactlyOne
    ((), 1)

runState'Test :: TestTree
runState'Test =
  testCase "runState'" $ runState' (state' . runState $ put 1) 0 @?= ((), 1)

execTTest :: TestTree
execTTest =
  testCase "execTTest" $ execT (StateT $ \s -> Full ((), s + 1)) 2 @?= Full 3

exec'Test :: TestTree
exec'Test = testCase "exec'Test" $ exec' (state' $ \s -> ((), s + 1)) 2 @?= 3
...
eval'Test :: TestTree
eval'Test =
  testCase "eval'Test" $ eval' (state' $ \s -> (even s, s + 1)) 5 @?= False
...
distinct'Test = testGroup
  "distinct'"
  [ testCase "removes duplicate 'c's"
  $   distinct' (listh "abcdcefcghi")
  @?= listh ['a' .. 'i']
...
distinctFTest = testGroup
  "distinctF"
  [ testCase "Full case" $ distinctF (listh [1, 2, 3, 2, 1]) @?= Full
    (listh [1, 2, 3])
...
optionalTApplicativeApplyTest = testGroup
  "(<*>) for OptionalT"
  [ testCase "one"
  ...
  , testCase "five"
    $ let ot = OptionalT (Empty :. Nil) <*> OptionalT (Full 1 :. Full 2 :. Nil)
      in  runOptionalT ot @?= (Empty :. Nil :: List (Optional Int))
  , testCase "six"
    $ let ot = OptionalT (Full (+ 1) :. Empty :. Nil)
            <*> OptionalT (Full 1 :. Full 2 :. Nil)
      in  runOptionalT ot @?= (Full 2 :. Full 3 :. Empty :. Nil)
  , testCase "seven"
    $ let ot = OptionalT (Full (+ 1) :. Full (+ 2) :. Nil)
            <*> OptionalT (Full 1 :. Empty :. Nil)
...
optionalTMonadTest =
  testCase "(=<<) for OptionalT"
    $ let ot = (\a -> OptionalT (Full (a + 1) :. Full (a + 2) :. Nil))
            =<< OptionalT (Full 1 :. Empty :. Nil)
      in  runOptionalT ot @?= (Full 2 :. Full 3 :. Empty :. Nil)

loggerFunctorTest :: TestTree
loggerFunctorTest =
  testCase "(<$>) for Logger" $ (+ 3) <$> Logger (1 :. 2 :. Nil) 3 @?= Logger
    (1 :. 2 :. Nil)
    6

loggerApplicativeTest :: TestTree
loggerApplicativeTest = testGroup
  "Logger Applicative"
  [ testCase "pure" $ (pure "table" :: Logger Int P.String) @?= Logger Nil
                                                                       "table"
  , testCase "<*>"
  $   Logger (1 :. 2 :. Nil)           (+ 7)
  <*> Logger (3 :. 4 :. Nil)           3
...
loggerMonadTest =
  testCase "(=<<) for Logger"
    $   ((\a -> Logger (4 :. 5 :. Nil) (a + 3)) =<< Logger (1 :. 2 :. Nil) 3)
    @?= Logger (1 :. 2 :. 4 :. 5 :. Nil) 6

log1Test :: TestTree
log1Test = testCase "log1" $ log1 1 2 @?= Logger (1 :. Nil) 2

distinctGTest :: TestTree
distinctGTest = testGroup
  "distinctG"
  [ testCase "Full case"
    $ let
        expected = Logger
