...
headOrTest = testGroup
  ...
  , testProperty "headOr on empty list always the default"
    $ \x -> (x :: Integer) `headOr` Nil == x
  ]

productTest :: TestTree
productTest = testGroup
  "productTest"
...
mapTest = testGroup
  ...
  , testProperty "map id is id" $ \x -> map id x == (x :: List Integer)
  ]

filterTest :: TestTree
filterTest = testGroup
  "filter"
  ...
  , testProperty "filter (const True) is identity"
    $ \x -> filter (const True) x == (x :: List Integer)
  , testProperty "filter (const False) is the empty list"
    $ \x -> filter (const False) x == (Nil :: List Integer)
  ]

appendTest :: TestTree
appendTest = testGroup
  "(++)"
  [ testCase "(1..6)" $ (1 :. 2 :. 3 :. Nil) ++ (4 :. 5 :. 6 :. Nil) @?= listh
    [1, 2, 3, 4, 5, 6]
  ...
  , testProperty "append to empty list" $ \x -> x ++ Nil == (x :: List Integer)
  ]

flattenTest :: TestTree
flattenTest = testGroup
  "flatten"
  [ testCase "(1..9)"
  $   flatten
        (  (1 :. 2 :. 3 :. Nil)
  ...
  , testProperty "sum of lengths == length of flattened"
    $ \x -> sum (map length x) == length (flatten (x :: List (List Integer)))
  ]

flatMapTest :: TestTree
flatMapTest = testGroup
  "flatMap"
  [ testCase "lists of Integer"
  $   flatMap (\x -> x :. x + 1 :. x + 2 :. Nil) (1 :. 2 :. 3 :. Nil)
  @?= listh [1, 2, 3, 2, 3, 4, 3, 4, 5]
  ...
  , testProperty "flatMap id == flatten"
    $ \x -> flatMap id x == flatten (x :: List (List Integer))
  ]

flattenAgainTest :: TestTree
flattenAgainTest = testGroup
  "flattenAgain"
  [ testProperty "lists of Integer"
      $ \x -> flatten x == flattenAgain (x :: List (List Integer))
  ]


seqOptionalTest :: TestTree
seqOptionalTest = testGroup
  "seqOptional"
  [ testCase "all Full" $ seqOptional (Full 1 :. Full 10 :. Nil) @?= Full
    (1 :. 10 :. Nil)
  , testCase "empty list"
...
findTest = testGroup
  "find"
  [ testCase "find no matches" $ find even (1 :. 3 :. 5 :. Nil) @?= Empty
  , testCase "empty list" $ find even Nil @?= Empty
  , testCase "find only even" $ find even (1 :. 2 :. 3 :. 5 :. Nil) @?= Full 2
  , testCase "find first, not second even"
  $   find even (1 :. 2 :. 3 :. 4 :. 5 :. Nil)
  @?= Full 2
  , testCase "find on infinite list" $ find (const True) infinity @?= Full 0
  ]

lengthGT4Test :: TestTree
lengthGT4Test = testGroup
  "lengthGT4"
  [ testCase "list of length 3" $ lengthGT4 (1 :. 3 :. 5 :. Nil) @?= False
  , testCase "list of length 4" $ lengthGT4 (1 :. 2 :. 3 :. 4 :. Nil) @?= False
  , testCase "empty list" $ lengthGT4 Nil @?= False
  , testCase "list of length 5"
  $   lengthGT4 (1 :. 2 :. 3 :. 4 :. 5 :. Nil)
  @?= True
  , testCase "infinite list" $ lengthGT4 infinity @?= True
  ]

reverseTest :: TestTree
reverseTest = testGroup
  "reverse"
  [ testCase "empty list" $ reverse Nil @?= (Nil :: List Integer)
  ...
  , testProperty "reverse then append is same as append then reverse"
    $ \x y -> reverse x ++ reverse y == (reverse (y ++ x) :: List Integer)
  , testProperty "reverse single element list is the list"
    $ \x -> reverse (x :. Nil) == (x :. Nil :: List Integer)
  ]

produceTest :: TestTree
produceTest = testGroup
  "produce"
  [ testCase "increment"
