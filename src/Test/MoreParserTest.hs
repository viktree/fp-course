...
import           Course.MoreParser              ( between
                                                , betweenCharTok
                                                , betweenSepbyComma
                                                , charTok
                                                , commaTok
...
import           Course.Parser                  ( ParseResult(Result)
                                                , character
                                                , is
                                                , isErrorResult
                                                , lower
...
spacesTest :: TestTree
spacesTest = testGroup
  "spacesTest"
  [ testCase "can parse zero spaces" $ parse spaces "abc" @?= Result "abc" ""
  , testCase "can parse single space" $ parse spaces " abc" @?= Result "abc" " "
  , testCase "can parse multiple spaces" $ parse spaces "   abc" @?= Result
    "abc"
    "   "
  ]
...
tokTest = testGroup
  ...
  $   parse (tok (is 'a')) "abc"
  @?= Result "bc" 'a'
  , testCase "can parse single space" $ parse (tok (is 'a')) "a bc" @?= Result
    "bc"
    'a'
...
charTokTest = testGroup
  "charTokTest"
  [ assertBool "fails when character does not match"
    $ isErrorResult (parse (charTok 'a') "dabc")
  , testCase "parses matching character" $ parse (charTok 'a') "abc" @?= Result
    "bc"
    'a'
...
commaTokTest = testGroup
  "commaTokTest"
  [ assertBool "fails when character is not a comma"
    $ isErrorResult (parse commaTok "1,23")
  , testCase "parses leading comma" $ parse commaTok ",123" @?= Result "123" ','
...
quoteTest = testGroup
  ...
  , testCase "parses single quote" $ parse quote "'abc" @?= Result "abc" '\''
  , testCase "parses double quote" $ parse quote "\"abc" @?= Result "abc" '"'
  ]

stringTest :: TestTree
stringTest = testGroup
  "stringTest"
  ...
  @?= Result "def" "abc"
  , testCase "parses matching string" $ parse (string "abc") "abc" @?= Result
    ""
    "abc"
  ]

stringTokTest :: TestTree
stringTokTest = testGroup
  "stringTokTest"
  [ assertBool "fails when string is not matched"
...
betweenTest = testGroup
  ...
  $   parse (between (is '[') (is ']') digits1) "[123]"
  @?= Result "" "123"
  ]

betweenCharTokTest :: TestTree
betweenCharTokTest = testGroup
  "betweenCharTokTest"
  ...
  $   parse (betweenCharTok '[' ']' digits1) "[123]"
  @?= Result "" "123"
  ]
...
hexTest = testGroup
  ...
  , testCase "succeeds on valid hex value" $ parse hex "0010" @?= Result
    ""
    '\DLE'
  ]
...
hexuTest = testGroup
  "hexuTest"
  [ assertBool "fails when no u at start" $ isErrorResult (parse hexu "0010")
  , assertBool "fails when not 4 hex digits after u"
    $ isErrorResult (parse hexu "u010")
  , assertBool "fails on invalid hex digit" $ isErrorResult (parse hexu "u0axf")
  , testCase "succeeds on valid input --- u0010" $ parse hexu "u0010" @?= Result
    ""
    '\DLE'
  , testCase "succeeds on valid input --- u0a1f" $ parse hexu "u0a1f" @?= Result
    ""
    '\2591'
  ]

sepby1Test :: TestTree
sepby1Test = testGroup
  "sepby1Test"
...
eofTest = testGroup
  "eofTest"
  [ assertBool "fails when still input left" $ isErrorResult (parse eof "abc")
  , testCase "succeeds when no input left" $ parse eof "" @?= Result "" ()
  ]

satisfyAllTest :: TestTree
satisfyAllTest = testGroup
  "satisfyAllTest"
...
satisfyAnyTest = testGroup
  ...
  $   parse (satisfyAny (isUpper :. (/= 'X') :. Nil)) "ABc"
  @?= Result "Bc" 'A'
...
betweenSepbyCommaTest = testGroup
  "betweenSepbyCommaTest"
