...
import           Course.Core
import           Course.JsonParser              ( jsonArray
                                                , jsonFalse
                                                , jsonNull
                                                , jsonNumber
                                                , jsonObject
...
jsonStringTest = testGroup
  "jsonString"
  [ testCase "parse whole ASCII input" $ parse jsonString "\" abc\"" @?= Result
    ""
    " abc"
...
jsonNumberTest = testGroup
  "jsonNumber"
  [ testCase "positive whole" $ parse jsonNumber "234" @?= Result "" (234 % 1)
  , testCase "negative whole" $ parse jsonNumber "-234" @?= Result
    ""
    ((-234) % 1)
  , testCase "positive decimal" $ parse jsonNumber "123.45" @?= Result
    ""
    (2469 % 20)
  , testCase "negative whole (2)" $ parse jsonNumber "-123" @?= Result
    ""
    ((-123) % 1)
  , testCase "negative decimal" $ parse jsonNumber "-123.45" @?= Result
    ""
    ((-2469) % 20)
  , testCase "negative sign on its own is error"
  $   isErrorResult (parse jsonNumber "-")
  @?= True
  , testCase "alphabetic characters is error"
  $   isErrorResult (parse jsonNumber "abc")
  @?= True
  ]

jsonTrueTest :: TestTree
jsonTrueTest = testGroup
  "jsonTrue"
  [ testCase "parses true" $ parse jsonTrue "true" @?= Result "" "true"
  , testCase "TRUE (caps) is an error"
  $   isErrorResult (parse jsonTrue "TRUE")
  @?= True
  ]

jsonFalseTest :: TestTree
jsonFalseTest = testGroup
  "jsonFalse"
  [ testCase "parses false" $ parse jsonFalse "false" @?= Result "" "false"
  , testCase "FALSE (caps) is an error"
  $   isErrorResult (parse jsonFalse "FALSE")
  @?= True
  ]

jsonNullTest :: TestTree
jsonNullTest = testGroup
  "jsonNull"
  [ testCase "parses null" $ parse jsonNull "null" @?= Result "" "null"
  , testCase "NULL (caps) is an error"
  $   isErrorResult (parse jsonNull "NULL")
  @?= True
  ]

jsonArrayTest :: TestTree
jsonArrayTest = testGroup
  "jsonArray"
  [ testCase "[]" $ parse jsonArray "[]" @?= Result "" Nil
  , testCase "[true]" $ parse jsonArray "[true]" @?= Result "" (JsonTrue :. Nil)
  , testCase "[true, \"abc\"]" $ parse jsonArray "[true, \"abc\"]" @?= Result
    ""
    (JsonTrue :. JsonString "abc" :. Nil)
  , testCase "[true, \"abc\", []]"
  ...
  , testCase "[true, \"abc\", [false]]"
    $ let result = Result
            ""
            (JsonTrue :. JsonString "abc" :. JsonArray (JsonFalse :. Nil) :. Nil
            )
      in  parse jsonArray "[true, \"abc\", [false]]" @?= result
  ]

jsonObjectTest :: TestTree
jsonObjectTest = testGroup
  "jsonObject"
  [ testCase "empty" $ parse jsonObject "{}" @?= Result "" Nil
  , testCase "one key" $ parse jsonObject "{ \"key1\" : true }" @?= Result
    ""
    (("key1", JsonTrue) :. Nil)
  , testCase "two keys"
  ...
  , testCase "two keys and left over input"
    $ let result =
      ...
      in  parse jsonObject "{ \"key1\" : true , \"key2\" : false } xyz"
            @?= result
  ]

jsonValueTest :: TestTree
jsonValueTest = testGroup
  "jsonValue"
  [ testCase "true" $ parse jsonValue "true" @?= Result "" JsonTrue
  , testCase "object"
    $ let result = Result
            ""
            (  ("key1", JsonTrue)
      ...
      in  parse jsonObject "{ \"key1\" : true , \"key2\" : [7, false] }"
            @?= result
