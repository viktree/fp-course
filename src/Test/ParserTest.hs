...
constantParserTest = testGroup
  "constantParserTest"
  [ testCase "can return error result"
  $   parse (constantParser (UnexpectedEof :: ParseResult Int)) "abc"
  @?= (UnexpectedEof :: ParseResult Int)
  , testCase "can return ParseResult"
  $   parse (constantParser (Result "xyz" 4)) "abc"
  @?= Result "xyz" 4
  ]

characterTest :: TestTree
characterTest = testGroup
  "characterTest"
...
functorTest = testGroup
  "functorTest"
  [ testCase "toUpper <$>" $ parse (toUpper <$> character) "amz" @?= Result
      "mz"
      'A'
  ]

valueParserTest :: TestTree
valueParserTest = testGroup
  "valueParserTest"
...
alternativeParserTest = testGroup
  "alternativeParserTest"
...
monadTest = testGroup
  ...
  [ assertBool "first parse fails" $ isErrorResult
    (parse
      ((\c -> if c == 'x' then character else valueParser 'v') =<< character)
      ""
    )
  , assertBool "second parse fails" $ isErrorResult
    (parse
      ((\c -> if c == 'x' then character else valueParser 'v') =<< character)
      "x"
    )
...
applicativeTest = testGroup
  ...
  [ testCase "pure" $ parse (pure 'a' :: Parser Char) "xyz" @?= Result "xyz" 'a'
  , testCase "pure an Optional value"
  $   parse (pure (Full 5) :: Parser (Optional Int)) "xyz"
  @?= Result "xyz" (Full 5)
  , testCase "pure toUpper <*>"
  $   parse (pure toUpper <*> valueParser 'a') "xyz"
  @?= Result "xyz" 'A'
  , testCase "pure show <*>"
  $   parse (pure show <*> valueParser 599) "xyz"
  @?= Result "xyz" "599"
  , testCase "append character <*>"
  $   parse (((\a b -> a :. b :. Nil) <$> character) <*> character) "abxyz"
  @?= Result "xyz" "ab"
  ]

satisfyTest :: TestTree
satisfyTest = testGroup
  "satisfyTest"
...
digitTest = testGroup
  ...
  , assertBool "is error when character not digit" $ isErrorResult $ parse
    digit
    "ABC"
...
spaceTest = testGroup
  "spaceTest"
  [ assertBool "fails when input empty" $ isErrorResult $ parse space ""
  , assertBool "fails when character not space" $ isErrorResult $ parse space
                                                                        "ABC"
...
listTest = testGroup
  "listTest"
  [ testCase "succeeds on empty input" $ parse (list character) "" @?= Result
    ""
    ""
...
spaces1Test = testGroup
  "spaces1Test"
  [ assertBool "fails on empty string" $ isErrorResult (parse spaces1 "")
  , testCase "consumes single space" $ parse spaces1 " " @?= Result "" " "
  , testCase "consumes multiple spaces" $ parse spaces1 "    abc" @?= Result
    "abc"
    "    "
  ]
...
lowerTest = testGroup
  ...
  , testCase "produces lowercase character" $ parse lower "aBC" @?= Result
    "BC"
    'a'
  ]
...
upperTest = testGroup
  ...
  , testCase "produces uppercase character" $ parse upper "Abc" @?= Result
    "bc"
    'A'
  ]
...
alphaTest = testGroup
  ...
  , testCase "produces alpha character" $ parse alpha "A45" @?= Result "45" 'A'
  ]

sequenceParserTest :: TestTree
sequenceParserTest = testGroup
  "sequenceParserTest"
  [ assertBool "fails on first failing parser" $ isErrorResult
...
ageParserTest = testGroup
  ...
  , testCase "parses valid age" $ parse ageParser "120" @?= Result "" 120
  ]

firstNameParserTest :: TestTree
firstNameParserTest = testGroup
  "firstNameParserTest"
  [ assertBool "fails on first name that doesn't start with a capital"
    $ isErrorResult (parse firstNameParser "abc")
  , testCase "parses valid first name" $ parse firstNameParser "Abc" @?= Result
    ""
    "Abc"
  ]

surnameParserTest :: TestTree
surnameParserTest = testGroup
  "surnameParserTest"
...
phoneBodyParserTest = testGroup
  ...
  $   parse phoneBodyParser "123-a456"
  @?= Result "a456" "123-"
  ]

phoneParserTest :: TestTree
phoneParserTest = testGroup
  "phoneParserTest"
...
personParserTest = testGroup
  "personParserTest"
  [ assertBool "fails on empty string" $ isErrorResult (parse personParser "")
