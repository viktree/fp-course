...
exactlyOneTest = testGroup
  "ExactlyOne instance"
  [ testProperty "pure == ExactlyOne"
    $ \x -> pure x == ExactlyOne (x :: Integer)
...
listTest = testGroup
  ...
  , testCase "<*>" $ (+ 1) :. (* 2) :. Nil <*> listh [1, 2, 3] @?= listh
    [2, 3, 4, 2, 4, 6]
  ]

haveFmapTest :: TestTree
haveFmapTest = testGroup
  "lift1"
  [ testCase "ExactlyOne" $ lift1 (+ 1) (ExactlyOne 2) @?= ExactlyOne
    (3 :: Integer)
  , testCase "empty List" $ lift1 (+ 1) Nil @?= Nil
  , testCase "List" $ lift1 (+ 1) (listh [1, 2, 3]) @?= listh [2, 3, 4]
  ]

optionalTest :: TestTree
optionalTest = testGroup
  "Optional instance"
  [ testProperty "pure" $ \x -> pure x == Full (x :: Integer)
  , testCase "Full <*> Full" $ Full (+ 8) <*> Full 7 @?= Full 15
  , testCase "Empty <*> Full"
  $   Empty
  <*> Full "tilt"
  @?= (Empty :: Optional Integer)
  , testCase "Full <*> Empty" $ Full (+ 8) <*> Empty @?= Empty
  ]

functionTest :: TestTree
functionTest = testGroup
  "Function instance"
  [ testCase "addition" $ ((+) <*> (+ 10)) 3 @?= 16
  , testCase "more addition" $ ((+) <*> (+ 5)) 3 @?= 11
  , testCase "even more addition" $ ((+) <*> (+ 5)) 1 @?= 7
  , testCase "addition and multiplication" $ ((*) <*> (+ 10)) 3 @?= 39
  , testCase "more addition and multiplcation" $ ((*) <*> (+ 2)) 3 @?= 15
...
lift2Test = testGroup
  ...
  @?= listh [5, 6, 6, 7, 7, 8]
  , testCase "+ over Optional - all full" $ lift2 (+) (Full 7) (Full 8) @?= Full
    15
  ...
  $   lift2 (+) (Full 7) Empty
  @?= Empty
  , testCase "+ over functions" $ lift2 (+) length sum (listh [4, 5, 6]) @?= 18
  ]
...
lift1Test = testGroup
  "lift1"
  [ testCase "+ over ExactlyOne" $ lift1 (+ 1) (ExactlyOne 2) @?= ExactlyOne 3
  , testCase "+ over empty List" $ lift1 (+ 1) Nil @?= Nil
  , testCase "+ over List"
  $   lift1 (+ 1) (1 :. 2 :. 3 :. Nil)
  @?= 2
  :.  3
...
rightApplyTest = testGroup
  "rightApply"
  [ testCase "*> over List" $ listh [1, 2, 3] *> listh [4, 5, 6] @?= listh
    [4, 5, 6, 4, 5, 6, 4, 5, 6]
  , testCase "*> over List" $ listh [1, 2] *> listh [4, 5, 6] @?= listh
    [4, 5, 6, 4, 5, 6]
  , testCase "another *> over List" $ listh [1, 2, 3] *> listh [4, 5] @?= listh
    [4, 5, 4, 5, 4, 5]
  , testCase "*> over Optional" $ Full 7 *> Full 8 @?= Full 8
...
leftApplyTest = testGroup
  ...
  <*  (4 :. 5 :. Nil)
  @?= listh [1, 1, 2, 2, 3, 3]
  , testCase "<* over Optional" $ Full 7 <* Full 8 @?= Full 7
  , testProperty "<* over List property" $ \a b c x y z ->
...
sequenceTest = testGroup
  ...
  @?= Empty
  , testCase "Optional" $ sequence (Full 7 :. Full 8 :. Nil) @?= Full
    (listh [7, 8])
  , testCase "(->)" $ sequence ((* 10) :. (+ 2) :. Nil) 6 @?= listh [60, 8]
  ]

replicateATest :: TestTree
replicateATest = testGroup
  "replicateA"
  [ testCase "ExactlyOne" $ replicateA 4 (ExactlyOne "hi") @?= ExactlyOne
    (listh ["hi", "hi", "hi", "hi"])
  , testCase "Optional - Full" $ replicateA 4 (Full "hi") @?= Full
    (listh ["hi", "hi", "hi", "hi"])
  , testCase "Optional - Empty"
  $   replicateA 4 Empty
  @?= (Empty :: Optional (List Integer))
  , testCase "(->)" $ replicateA 4 (* 2) 5 @?= listh [10, 10, 10, 10]
  , testCase "List"
    $ let expected = listh <$> listh
            [ "aaa"
            , "aab"
            , "aac"
            , "aba"
...
filteringTest = testGroup
  ...
  , testCase "List"
    $ let expected = listh <$> listh
      ...
      in  filtering (const $ True :. True :. Nil) (1 :. 2 :. 3 :. Nil)
            @?= expected
  ]
