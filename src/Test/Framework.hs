...
import           Test.Framework.Random          ( StdGen
                                                , mkStdGen
                                                , split
                                                )
 ...
 where
  ...
  go s (Single s' a) =
    let
      ...
      printFailure e =
        putStrLn "" >> putStrLn ("FAILED: " <> qName) >> putStrLn
          (indent 2 (show e))

      printResult (Failure e) = printFailure e
      printResult Success     = putStrLn $ "PASSED: " <> qName
    in
      printResult a `catch` \(e :: SomeException) -> printFailure e
...
a @?= b =
  let msg = "Expected " <> show b <> " but got " <> show a
  in  bool (Failure msg) Success (a == b)
