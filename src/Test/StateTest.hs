...
import           Course.State                   ( State(State)
                                                , distinct
                                                , eval
                                                , exec
                                                , findM
...
execTest :: TestTree
execTest = testProperty "exec" $ \(Unshowable f) s -> exec (State f) s
  == snd (runState (State (f :: Int -> (Int, Int))) (s :: Int))

evalTest :: TestTree
evalTest = testProperty "eval" $ \(Unshowable f) s -> eval (State f) s
  == fst (runState (State (f :: Int -> (Int, Int))) (s :: Int))

getTest :: TestTree
getTest = testCase "get" $ runState get 0 @?= (0, 0)

putTest :: TestTree
putTest = testCase "put" $ runState (put 1) 0 @?= ((), 1)

functorTest :: TestTree
functorTest =
  testCase "(<$>)" $ runState ((+ 1) <$> State (\s -> (9, s * 2))) 3 @?= (10, 6)
...
monadTest = testGroup
  "Monad"
  [ testCase "(=<<)" $ runState (const (put 2) =<< put 1) 0 @?= ((), 2)
  , testCase "correctly produces new state and value"
  $   runState
        ((\a -> State (\s -> (a + s, 10 + s))) =<< State (\s -> (s * 2, 4 + s)))
        2
  @?= (10, 16)
  , testCase "(>>=)"
...
firstRepeatTest = testGroup
  "firstRepeat"
  [ testCase "'x' is the only repeat" $ firstRepeat (listh "abxdexghi") @?= Full
    'x'
  , testCase "'x' is the first repeat" $ firstRepeat (listh "abxdexgg") @?= Full
    'x'
  , testCase "no repeats" $ firstRepeat (listh ['a' .. 'z']) @?= Empty
...
distinctTest = testGroup
  "distinct"
  [ testCase "No repeats" $ let cs = listh ['a' .. 'z'] in distinct cs @?= cs
  , testCase "Every element repeated"
    $ let cs = listh ['a' .. 'z']
      in  distinct (flatMap (\x -> x :. x :. Nil) cs) @?= cs
  , testProperty "No repeats after distinct"
    $ \xs -> firstRepeat (distinct (xs :: List Integer)) == Empty
  , testProperty "Every element repeated" $ \xs -> distinct (xs :: List Integer)
    == distinct (flatMap (\x -> x :. x :. Nil) xs)
  ]

isHappyTest :: TestTree
isHappyTest = testGroup
  "isHappy"
